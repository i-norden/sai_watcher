...
func (car CupActionsRepository) CreateCupAction(cupAction CupActionModel, logID int64) error {
	...
	_, err := car.DB.Exec(
		`INSERT INTO maker.cup_action (log_id, id, tx, act, arg, lad, ink, art, ire, block, deleted)
                SELECT $1, $2, $3, $4, NULLIF($5, ''), $6, $7::NUMERIC, $8::NUMERIC, $9::NUMERIC, $10, $11
                WHERE NOT EXISTS (SELECT log_id FROM maker.cup_action WHERE log_id = $1)`,
		logID, id, tx, act, arg, lad, ink, art, ire, block, deleted)
	if err != nil {
		return err
	}
	return nil
