...
type CupModifiedHandler struct {
	...
	Fetcher                cup_actions.FetcherInterface
...
func NewCupModifiedHandler(db *postgres.DB, blockchain core.Blockchain) shared.Handler {
	...
	fetcher := cup_actions.Fetcher{blockchain}
	for _, filter := range CupModifiedFilters {
		fr.CreateFilter(filter)
	}
	return &CupModifiedHandler{
...
func (cmh CupModifiedHandler) Execute() error {
	for _, filter := range CupModifiedFilters {
...
func createCupAction(cmh CupModifiedHandler, watchedEvent *core.WatchedEvent) {
