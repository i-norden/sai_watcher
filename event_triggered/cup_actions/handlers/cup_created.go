...
func NewCupCreatedHandler(db *postgres.DB, blockchain core.Blockchain) shared.Handler {
	var transformer shared.Handler
	we := repositories.WatchedEventRepository{DB: db}
	car := cup_actions.CupActionsRepository{DB: db}
	transformer = &CupCreatedHandler{
...
func (cch CupCreatedHandler) Execute() error {
