...
var _ = Describe("Cup Modified transformer", func() {
	It("fetches watched events for cup modified log filters", func() {
		...
		transformer := handlers.CupModifiedHandler{
			Blockchain:             nil,
			WatchedEventRepository: &mockEventsRepo,
			CupActionsRepository:   &test_helpers.MockCupActionsRepository{},
			Fetcher:                &test_helpers.MockFetcher{},
		}

		transformer.Execute()

		Expect(len(mockEventsRepo.EventNames)).To(Equal(7))
		Expect(mockEventsRepo.EventNames[0]).To(Equal(handlers.CupModifiedFilters[0].Name))
		Expect(mockEventsRepo.EventNames[1]).To(Equal(handlers.CupModifiedFilters[1].Name))
		Expect(mockEventsRepo.EventNames[2]).To(Equal(handlers.CupModifiedFilters[2].Name))
		Expect(mockEventsRepo.EventNames[3]).To(Equal(handlers.CupModifiedFilters[3].Name))
		Expect(mockEventsRepo.EventNames[4]).To(Equal(handlers.CupModifiedFilters[4].Name))
		Expect(mockEventsRepo.EventNames[5]).To(Equal(handlers.CupModifiedFilters[5].Name))
		Expect(mockEventsRepo.EventNames[6]).To(Equal(handlers.CupModifiedFilters[6].Name))
	})
	...
	It("fetches cup data for corresponding watched event", func() {
		...
		mockFetcher := test_helpers.MockFetcher{}
		transformer := handlers.CupModifiedHandler{
			CupActionsRepository:   &test_helpers.MockCupActionsRepository{},
	...
	It("persists cup action with fetched cup data", func() {
		...
		returnEvents := []*core.WatchedEvent{
			{
				...
				Name:        "",
				BlockNumber: blockNumber,
				Address:     handlers.TubContractAddress,
				TxHash:      transactionHash,
				Index:       0,
				Topic0:      handlers.LockActionHex,
		...
		mockFetcher := test_helpers.MockFetcher{ReturnVal: returnVal}
