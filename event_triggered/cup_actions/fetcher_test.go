...
var _ = Describe("Fetcher", func() {
	It("fetches contract data", func() {
		...
		fetcher := cup_actions.Fetcher{&mockBlockchain}
		methodArg := "methodArg"
		...
		Expect(mockBlockchain.AbiJSONs[0]).To(Equal(cup_actions.CupsContractABI))
		Expect(len(mockBlockchain.Addresses)).To(Equal(1))
		Expect(mockBlockchain.Addresses[0]).To(Equal(cup_actions.CupsContractAddress))
